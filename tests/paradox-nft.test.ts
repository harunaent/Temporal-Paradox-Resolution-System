...
describe('Paradox NFT Contract', () => {
  ...
  describe('mint', () => {
    it('should mint an NFT successfully', async () => {
      ...
      const uri = 'https://example.com/paradox/1';
      
      mockContractCall.mockResolvedValue({ value: 1 }); // Assuming 1 is the new token ID
      
      const result = await mockContractCall('paradox-nft', 'mint', [recipient, uri]);
      
      expect(result.value).toBe(1);
      expect(mockContractCall).toHaveBeenCalledWith('paradox-nft', 'mint', [recipient, uri]);
    });
  });
  ...
  describe('transfer', () => {
    it('should transfer an NFT successfully', async () => {
      ...
      const result = await mockContractCall('paradox-nft', 'transfer', [tokenId, sender, recipient]);
      
      expect(result.value).toBe(true);
      expect(mockContractCall).toHaveBeenCalledWith('paradox-nft', 'transfer', [tokenId, sender, recipient]);
    });
    ...
    it('should fail if not called by the token owner', async () => {
      ...
      await expect(mockContractCall('paradox-nft', 'transfer', [tokenId, sender, recipient]))
  ...
  describe('get-token-uri', () => {
    it('should return the correct token URI', async () => {
      const tokenId = 1;
      const expectedUri = 'https://example.com/paradox/1';
      
      mockContractCall.mockResolvedValue({ value: expectedUri });
      
      const result = await mockContractCall('paradox-nft', 'get-token-uri', [tokenId]);
      
      expect(result.value).toBe(expectedUri);
      expect(mockContractCall).toHaveBeenCalledWith('paradox-nft', 'get-token-uri', [tokenId]);
    });
    ...
    it('should fail for non-existent token', async () => {
      ...
      await expect(mockContractCall('paradox-nft', 'get-token-uri', [tokenId]))
  ...
  describe('get-owner', () => {
    it('should return the correct token owner', async () => {
      ...
      const result = await mockContractCall('paradox-nft', 'get-owner', [tokenId]);
      
      expect(result.value).toBe(expectedOwner);
      expect(mockContractCall).toHaveBeenCalledWith('paradox-nft', 'get-owner', [tokenId]);
    });
